...
const CompaniesPriceChart = () => {
  const { data: companiesResponse, isLoading } = useCompaniesQuery({ page: 1, limit: 6, sortBy: 'capital', sortOrder: 'desc' });
  ...
  if (chartData.length === 0) {
    return <div className="text-center py-4 text-gray-500">No companies to display</div>;
  }
  ...
  return (
    <div style={{ width: '100%' }}>
      <p className="text-sm text-gray-600 mb-2">Click each bar to see details</p>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
          ...
          />
          <Tooltip
            formatter={(value: number) => [`$${shortenNumber(value)}`, 'Capital']}
      ...
      <div className="mt-3 p-3 bg-dark-800 rounded-lg">
        <p className="text-sm font-medium text-white-800">
          Capital of "{activeItem?.fullName}": <span className="text-blue-600">${shortenNumber(activeItem?.capital || 0)}</span>
        </p>
      </div>
    </div>
  );
